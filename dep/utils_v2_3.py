...
def validate_lists(list_one, list_two, scope):
    ...
    if not is_valid:
        return f'{FATAL} Invalid matching detected, {len(list_one)} sources but {len(list - list_two)} destinations'
    return (f'{INFO if is_valid else ERROR} The {len(list_one)} found {scope} have {len(list_two)} matches and are therefore ' +
